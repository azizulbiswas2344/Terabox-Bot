            ...
            if data.get("ok"):
                ...
                with open(video_filename, "wb") as file:
                   for chunk in response.iter_content(chunk_size=10 * 1024 * 1024):  # Save in chunks
                       file.write(chunk)
                       current_size += len(chunk)
                       await progress_for_pyrogram(current_size, video_size, f"<blockquote>♻ ᴜᴘʟᴏᴀᴅɪɴɢ ᴠɪᴅᴇᴏ ᴛᴏ sᴇʀᴠᴇʀ</blockquote>\n<blockquote><code>{video_title}</code></blockquote>", progress_message2, start_time)
                await client.send_chat_action(message.chat.id, enums.ChatAction.TYPING)
               ...
               # width, height, duration = await Mdata01(video_filename)
                ...
                final_stick = await message.reply_sticker("CAACAgIAAxkBAAEVtYFnbBxMYATxBLqL4Lv6bnA5LigRUgAC4gUAAj-VzApzZV-v3phk4DYE")
